...
def pgd_attack(model, inputs, labels, epsilon=0.03, alpha=0.01, num_steps=40):
    print("in pgd_attack")
    model.eval()
    fmodel = foolbox.models.PyTorchModel(model, bounds=(0, 1))
    attack = foolbox.attacks.LinfProjectedGradientDescentAttack(
    rel_stepsize=0.003333333333333333,
    abs_stepsize=alpha,
    steps=num_steps,
    random_start=True)
    print("before attack")
    adversarial_inputs = attack(fmodel, inputs, labels, epsilons=epsilon)
    model.train()
...
def evaluate_model(model, X):
    ...
    with torch.no_grad():
        ...
        prediction = (outputs >= 0.5).astype(int) # if outputs >= 0.5, prediction = 1
...
class OutputNetwork(nn.Module):
    def __init__(self, freeze_others=True): # freeze TRADES and standard while training output network
        ...
        self.mixing_layer_A = nn.Linear(1 + 10, 512, bias=True) # mix detector output with standard
        self.mixing_layer_B = nn.Linear(1 + 640, 512, bias=True) # mix detector output with TRADES
        ...
        self.model_A = nn.DataParallel(Wide_ResNet(28, 10, 0.3, 10), device_ids=[0]) # wrn_28_10
        A_ckpt = torch.load('./wide-resnet.pytorch/checkpoint/cifar10/wide-resnet-28x10.pt')
        ...
        self.detector = LogisticRegressionModel(3072)
        D_ckpt = torch.load("./SpectralAdversarialDefense/data/detectors/LR_fgsm_InputMFS_test_cif10.pt", map_location="cuda:0")
        self.detector.load_state_dict(D_ckpt)
        self.detector.cuda()
        self.detector.eval()
    ...
    def forward(self, X):
        ...
        def detector_preprocess(X):
            ...
            # X = X[i].unsqueeze_(0)
            X = cifar_normalize(X)
            # X = get_layer_feature_maps(X, fourier_act_layers)
            X = calculate_spectra(X) 
            characteristics = torch.asarray(X, dtype=torch.float32)
            # print(characteristics)
            return characteristics

        detector_input = detector_preprocess(X)
        model_A_output = self.model_A(X)
        # print("model_A_output")
        # print(model_A_output.shape)
        # print(model_A_output)
        model_B_output = self.model_B(X)
        # print("model_B_output")
        # print(model_B_output.shape)
        # print(model_B_output)
        scaler = MinMaxScaler().fit(detector_input)
        detector_input = scaler.transform(detector_input)
        detector_input = torch.tensor(detector_input, dtype = torch.float32)
        detector_output = evaluate_model(self.detector, detector_input.to("cuda:0"))[1]
        
        # print(detector_output)
        debug_output = evaluate_model(self.detector, detector_input.to("cuda:0"))[0]
        # print(detector_output)
        # print(debug_output)
        ...
        # print("model_A_flat")
        # print(model_A_output_flat.shape)
        # print(model_A_output_flat)
        # print("model_B_flat")
        # print(model_B_output_flat.shape)
        # print(model_B_output_flat)
        detector_output = torch.from_numpy(detector_output)
        # print(detector_output.shape)
        # print(detector_output)
        # detector_output = detector_output.unsqueeze(-1)
        # print(detector_output.shape)
        # print(detector_output)
        # print(detector_output.shape)
        # print(detector_output)
        # model_A_output_flat = torch.from_numpy(model_A_output_flat)
        # model_B_output_flat = torch.from_numpy(model_B_output_flat)
        detector_output = detector_output.to(torch.device("cuda:0"))
        # Concatenate detector_output with model_A_output and model_B_output
        mixed_output_A = torch.cat((detector_output, model_A_output_flat), dim=1)
        mixed_output_B = torch.cat((detector_output, model_B_output_flat), dim=1)
        # print(mixed_output_A.shape)

        # Pass through the mixing layers
        # mixed_output_A = torch.tensor(mixed_output_A, dtype=torch.float32)
        mixed_output_A = mixed_output_A.to(torch.float32)
        # mixed_output_B = torch.tensor(mixed_output_B, dtype=torch.float32)
        mixed_output_B = mixed_output_B.to(torch.float32)
        # print("mixed shape")
        # print(mixed_output_A.shape) #32 11
        # print(mixed_output_B.shape) #32 641 
        mixed_output_A = self.mixing_layer_A(mixed_output_A)
        mixed_output_B = self.mixing_layer_B(mixed_output_B)
        # print("mixed shape")
        # print(mixed_output_A.shape)
        # print(mixed_output_B.shape) 
...
val_loader = torch.utils.data.DataLoader(testset, batch_size=64, shuffle=False, **kwargs)
print(len(trainset))
print(len(testset))
...
print_every = 1
batch_size = 64  # Number of total samples in the batch
ratio = 0.5
num_to_attack = int(batch_size * ratio)
...
def train_clean_network():
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            indices_to_attack = np.random.choice(batch_size, num_to_attack, replace=False)
            if (i < 781):
                adversarial_inputs = pgd_attack(net, inputs[indices_to_attack], labels[indices_to_attack])
            mixed_inputs = inputs.clone()
            if (i < 781):
                mixed_inputs[indices_to_attack] = adversarial_inputs[0]
            # Forward pass
        ...
        with torch.no_grad():
            ...
            for data in val_loader:
                ...
                if correct > bestcorrect:
                    bestcorrect = correct
                    torch.save(net.state_dict(), './test_checkpoints_nodetector/best_checkpoint_withpgd_08240236.pt')
        print(f'Epoch [{epoch+1}], Validation Loss: {val_loss/len(val_loader):.4f}, Validation Accurcacy: {100*correct/total:.2f}%')
